...
async function showSeasonLanding() {
  ...
  container.innerHTML = seasons.map(s => {
    ...
    return `
      <div class="season-card${isActive ? '' : ' season-closed'}" ${isActive ? `data-action="selectSeason" data-id="${sid}"` : ''}>
...
async function loadReferenceData() {
  try {
    ...
    const { data: collections, error: collectionsError } = await supa
      .from('collections')
      .select('collection_id, collection_name, delivery_season, tab_order, status')
      .eq('status', 'active')
...
document.addEventListener('click', function(e) {
  ...
  else if (a === 'selectSeason') selectSeason(el.dataset.id);
